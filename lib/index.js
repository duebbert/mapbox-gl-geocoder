...
MapboxGeocoder.prototype = {
  ...
  onAdd: function(map) {
    ...
    this._geocodeComplete = this._geocodeComplete.bind(this);
  ...
  _geocode: function(searchInput) {
    ...
    var mbGeocoderReq, localGeocoderReq;
    var localGeocoderStatus = 'PENDING';
    var mbGeocoderStatus = 'PENDING';

    if (this.options.localGeocoderOnly){
      mbGeocoderReq = Promise.resolve([]); // empty results promise for later handling.
    }
    ...
    // make the request a reverseGeocode
      ...
      mbGeocoderReq = this.geocoderService.reverseGeocode(config).send();
    } else {
      config = extend(config, { query: searchInput });
      mbGeocoderReq = this.geocoderService.forwardGeocode(config).send();
    }

    // localGeocoder must be a function that returns a Promise
    // localGeocoder results should be an array of features
    if (this.options.localGeocoder) {
      localGeocoderReq = new Promise(function(resolve, reject){
        self.options.localGeocoder(searchInput).then(function(result){
          resolve(result);
        }).catch(function(err){
          reject(err);
        });
      ...
      localGeocoderReq = Promise.resolve([]); // empty results promise for later handling.
    }

    localGeocoderReq.then(function(results){
      localGeocoderStatus = results;
      if (mbGeocoderStatus !== 'PENDING'){
        self._geocodeComplete(localGeocoderStatus, mbGeocoderStatus, config);
      }
    });

    localGeocoderReq.catch(function(error){
      localGeocoderStatus = error;
      if (mbGeocoderStatus !== 'PENDING'){
        self._geocodeComplete(localGeocoderStatus, mbGeocoderStatus, config);
      }
    });

    mbGeocoderReq.then(function(results){
      mbGeocoderStatus = results;
      if (localGeocoderStatus !== 'PENDING'){
        self._geocodeComplete(localGeocoderStatus, mbGeocoderStatus, config);
      }
    });

    mbGeocoderReq.catch(function(error){
      mbGeocoderStatus = new Error(error);
      if (localGeocoderStatus !== 'PENDING'){
        self._geocodeComplete(localGeocoderStatus, mbGeocoderStatus, config);
      }
    });
  ...
  // redundant with _setResults
  _onQueryResult: function() {
  ...
  _setResults: function(results) {
    ...
    this._eventEmitter.emit('results', { features: results });
    this._typeahead.update(results);
  },

  _geocodeComplete: function(localStatus, mbStatus, config) {
    this._loadingEl.style.display = 'none';

    if (localStatus instanceof Error && mbStatus instanceof Error){
      this._eventEmitter.emit('error', { error: mbStatus });
      this._eventEmitter.emit('error', { error: localStatus });
      return
    } else if (localStatus instanceof Error){
      return
    } else if (mbStatus instanceof Error){
      // in the event of an error in the Mapbox Geocoding API still display results from the localGeocoder
      this._setResults(localStatus);
      this._eventEmitter.emit('error', { error: mbStatus });

      return
    } else {
      var res = {}
      if (mbStatus.statusCode == '200') {
        res = mbStatus.body;
      }
      res.features = res.features
        ? localStatus.concat(res.features)
        : localStatus;
      if (this.options.filter && res.features.length) {
        res.features = res.features.filter(this.options.filter);
      }
      this._setResults(res.features);

      res.config = config;
      if (this.fresh){ //autocomplete session starting
